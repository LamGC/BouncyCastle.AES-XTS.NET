...
public class XtsAesBufferedCipher : IBufferedCipher, IDisposable
{
    ...
    private AesEngine _dataEncEngine = new();
    ...
    {
        if (parameters is not XtsAesCipherParameters xtsParameters)
        {
            throw new ArgumentException("parameters must be of type XtsAESCipherParameters!");
        }
        
        CheckParameters(xtsParameters);
        
        _parameters = xtsParameters;
    ...
    private void Reset(bool resetParameters)
    {
        ...
        if (resetParameters)
        {
            ...
            _dataEncEngine = new AesEngine();
            _dataEncEngine.Init(_forEncryption, _parameters.Key1);
            _tweakGenerator = new XtsTweakStatefulGenerator(_parameters.Key2, _parameters.SectorSize, _parameters.SectorIndex);
        }

        CryptographicOperations.ZeroMemory(_unhandledBuffer);
    ...
    private void CheckState()
    {
        ...
        if (!_initialized)
        {
            throw new InvalidOperationException("Cipher must be initialized!");
        }
    }
    ...
    {
        ArgumentOutOfRangeException.ThrowIfNegative(inputLen);
        CheckState();
    ...
    private int ProcessBytes0(ReadOnlySpan<byte> input, Span<byte> output)
    {
        ...
        try
        {
            ...
            finally
            {
                ...
                CryptographicOperations.ZeroMemory(outputBlock);
            }
    ...
    private void ProcessXtsBlock(Span<byte> input, Span<byte> output)
    {
        ...
        try {
            ...
            _dataEncEngine.ProcessBlock(input, output);
    ...
    public byte[] ProcessByte(byte input)
    {
        ...
        ProcessBytes0(new ReadOnlySpan<byte>(ref input), output);
        return output;
    }
    ...
    {
        CheckState();
        return ProcessBytes0(new ReadOnlySpan<byte>(ref input), output.AsSpan(outOff));
    }
    ...
    public int ProcessByte(byte input, Span<byte> output)
    {
        ...
        var handledBytes = ProcessBytes0(new ReadOnlySpan<byte>(ref input), output);
    ...
    /// <exception cref="DataLengthException">当传入的数据量超过安全范围, 或在调用 DoFinal 传入最后数据时出现跨扇区问题, 就会抛出该异常.</exception>
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    private void CheckInput(ReadOnlySpan<byte> input, int offset, int length, bool isDoFinal = false)
    {
        ...
        {
            throw new DataLengthException($"The provided length ({length}) exceeds the safe range ({MaxInputSizeAtProcess}) for single processing.");
        }
    ...
    public int DoFinal(Span<byte> output)
    {
        ...
        // Tn-1
        var tweakN1 = _tweakGenerator.GetTweakAndMove();
        // Tn
        var tweakN = _tweakGenerator.CurrentTweak.ToArray();
        ...
        try {
            ...
            if (_forEncryption)
            {
                ...
                try
                {
                    ...
                    _dataEncEngine.ProcessBlock(pn1Block, tempC);
                    ...
                    _dataEncEngine.ProcessBlock(pnPaddedBlock, tempC);
                ...
                {
                    CryptographicOperations.ZeroMemory(cn);
                    CryptographicOperations.ZeroMemory(stolenData);
                }
            }
            else
            {
                ...
                try
                {
                    ...
                    _dataEncEngine.ProcessBlock(pn1Block, pnPaddedBlock);
                    XtsUtils.XorBlock(pnPaddedBlock, tweakN);
                    ...
                    _dataEncEngine.ProcessBlock(pnPaddedBlock, cn1Output);
                ...
                {
                    CryptographicOperations.ZeroMemory(cn1Output);
                }
            }
        ...
        {
            CryptographicOperations.ZeroMemory(pn1Block);
            CryptographicOperations.ZeroMemory(pnBlock);
            CryptographicOperations.ZeroMemory(tweakN1);
            CryptographicOperations.ZeroMemory(tweakN);
            CryptographicOperations.ZeroMemory(tempC);
            CryptographicOperations.ZeroMemory(pnPaddedBlock);
        }
