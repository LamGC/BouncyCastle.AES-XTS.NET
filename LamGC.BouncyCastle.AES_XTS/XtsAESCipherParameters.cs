...
public class XtsAesCipherParameters : ICipherParameters
{
    
    public XtsAesMode Mode { get; private set; }
    
    public KeyParameter Key1 { get; private set; }
    public KeyParameter Key2 { get; private set; }
    ...
    public XtsAesCipherParameters(XtsAesMode mode, byte[] dataEncryptKey, byte[] tweakCalcKey, ulong sectorSize,
        ulong sectorIndex = 0) : this(mode, new KeyParameter(dataEncryptKey), new KeyParameter(tweakCalcKey),
        sectorSize, sectorIndex)
    {
    ...
    /// </summary>
    /// <param name="mode">Cipher 加密模式, 参阅 XtsAESMode 说明.</param>
    /// <param name="dataEncryptKey">数据加解密密钥, 即 K1.</param>
    /// <param name="tweakCalcKey">Tweak 值生成密钥, 即 K2.</param>
    /// <param name="sectorSize">扇区大小, 必须是 16 的倍数(符合 AES 块大小).</param>
    /// <param name="sectorIndex">本次处理起始的扇区索引, 起始值为 0.</param>
    /// <exception cref="ArgumentException">当扇区大小不是 16 的倍数时抛出.</exception>
    public XtsAesCipherParameters(XtsAesMode mode, KeyParameter dataEncryptKey, KeyParameter tweakCalcKey, ulong sectorSize, ulong sectorIndex = 0)
    {
        if (sectorSize < 16)
        {
