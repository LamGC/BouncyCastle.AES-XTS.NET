...
public abstract class AbstractXtsTweakGenerator
{
    // 128 bits
    ...
    protected readonly AesEngine TweakCipher;
    
    protected AbstractXtsTweakGenerator(KeyParameter tweakCalcKey)
    {
        TweakCipher = new AesEngine();
        TweakCipher.Init(true, tweakCalcKey);
    }
...
public class XtsTweakGenerator(KeyParameter tweakCalcKey) : AbstractXtsTweakGenerator(tweakCalcKey)
{
    
    public byte[] CalculateTweak(ulong sectorIndex, ulong blockIndex)
    {
        var sectorIndexBytes = new byte[16];
        ...
        {
            CryptographicOperations.ZeroMemory(sectorIndexBytes);
        }
    }
    ...
    public byte[] CalculateTweak(byte[] sectorIndex128, ulong blockIndex)
    {
        ...
        TweakCipher.ProcessBlock(sectorIndex128, 0, t0, 0);
...
public class XtsTweakStatefulGenerator : AbstractXtsTweakGenerator, IDisposable
{
    ...
    public XtsTweakStatefulGenerator(KeyParameter tweakCalcKey, ulong sectorSize, ulong startSectorIndex = 0, ulong startBlockIndex = 0) : base(tweakCalcKey)
    {
    ...
    private void RecalculateSectorStartTweak()
    {
        ...
        TweakCipher.ProcessBlock(sectorIndexBytes, 0, _currentBlockTweak, 0);
        CryptographicOperations.ZeroMemory(sectorIndexBytes);
    }
    ...
    public void Dispose()
    {
        CryptographicOperations.ZeroMemory(_currentBlockTweak);
